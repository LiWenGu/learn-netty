...
public abstract class AbstractChannel extends DefaultAttributeMap implements Channel {
    ...
    protected AbstractChannel(Channel parent) {
        this.parent = parent;
        logger.info("注释三：1. id、unsafe、pipeline初始化");
    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        private void register0(ChannelPromise promise) {
            try {
                ...
                logger.info("注释三：3. invokeHandlerAddedIfNeeded，事件回调：com.imooc.netty.ch3.ServerHandler.handlerAdded");
                pipeline.invokeHandlerAddedIfNeeded();

                safeSetSuccess(promise);
                logger.info("注释三：3. 传播事件，事件回调：com.imooc.netty.ch3.ServerHandler.channelRegistered");
                pipeline.fireChannelRegistered();
                // Only fire a channelActive if the channel has never been registered. This prevents firing
                // multiple channel actives if the channel is deregistered and re-registered.
                if (isActive()) {
                    if (firstRegistration) {
                        logger.info("注释五：5. 第一次注册发射");
        ...
        public final void bind(final SocketAddress localAddress, final ChannelPromise promise) {
            ...
            if (!wasActive && isActive()) {
                logger.info("注释三：4. pipeline.fireChannelActivie()：传播事件");
        ...
        public final void flush() {
            ...
            logger.info("注释九：4. 添加刷新标志并设置写状态");
